...
Wp.N             = 2*floor(750/2);           % simulation time ( real simulation time is N-h )
...
while Wp.k<Wp.N/(2*Wp.h)
    ...
    if Wp.k==ap.sd;sr.u(3,Wp.k) = 0.0;end % ap.sd is time at which turbine 3 shuts down
    ...
    xmes                 = [squeeze(Wp.y(Wp.k,:,3:4)) uc];  %=[F P u_filtred]
